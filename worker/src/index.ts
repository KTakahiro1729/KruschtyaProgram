...
app.post('/api/sessions', async (c) => {
  ...
  const ts = nowMs();
  await c.env.DB.prepare('INSERT INTO sessions (id, owner_id, password, created_at, last_updated, mode) VALUES (?, ?, ?, ?, ?, ?)')
    .bind(sessionId, user.id, password, ts, ts, 'system').run();
  await c.env.DB.prepare('INSERT INTO session_tokens (id, session_id, password, created_at) VALUES (?, ?, ?, ?)')
    .bind(uuid(), sessionId, password, ts).run();
...
app.post('/api/sessions/:id/join', async (c) => {
  ...
  const ts = nowMs();
  const pid = uuid();
  await c.env.DB.prepare('INSERT INTO participants (id, session_id, name, created_at) VALUES (?, ?, ?, ?)').bind(pid, sessionId, name ?? 'Guest', ts).run();
...
app.post('/api/sessions/:id/palettes', async (c) => {
  ...
  for (const item of items) {
    await c.env.DB.prepare('INSERT INTO chat_palettes (id, participant_id, label, content, created_at) VALUES (?, ?, ?, ?, ?)')
      .bind(uuid(), participantId, item.label, item.content, ts).run();
  }
...
app.post('/api/sessions/:id/kp', async (c) => {
  ...
  await c.env.DB.prepare('UPDATE sessions SET mode = ?, manual_time = ?, current_time_offset = ? WHERE id = ?')
    .bind(mode ?? 'system', manualTime ?? null, offset ?? 0, sessionId).run();
  return c.json({ ok: true });
});
