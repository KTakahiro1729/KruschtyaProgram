<template>
    ...
    >
        ...
        >
            <div
                class="flex items-center gap-2 text-[11px] uppercase tracking-[0.16em] text-slate-400"
            >
                <span class="font-semibold text-slate-200"
                    >Kruschtya Session</span
                >
                ...
                <span
                    ...
                    :class="
                        kpRunning
                            ...
                            : 'bg-rose-900/40 text-rose-200 border border-rose-500/50'
                    "
                >
        ...
        <main
            ref="logRef"
            class="flex-1 overflow-y-auto px-3 py-2 space-y-1 bg-slate-900/60"
        >
            <section
            ...
            >
                <div
                    class="flex items-baseline gap-2 text-[11px] leading-tight text-slate-500"
                >
                    <span class="font-mono">{{
                        formatLogTime(msg.created_at)
                    }}</span>
                    <button
                    ...
                    <span :class="speakerClass(msg)">{{
                        msg.speaker_name || "名無しさん"
                    }}</span>
                </div>
                <div
                    class="mt-1 flex items-start justify-between gap-3 text-sm leading-tight text-slate-100"
                >
                    <span class="flex-1 whitespace-pre-wrap break-words">{{
                        msg.rendered_text || msg.raw_text
                    }}</span>
                    <span
                        v-if="diceBadge(msg)"
        ...
        <footer
            class="bg-slate-800/90 border-t border-slate-700 flex flex-col h-72 shadow-[0_-10px_30px_rgba(0,0,0,0.35)]"
        >
            <div
                class="flex items-center justify-between border-b border-slate-700 px-4 py-2 text-xs"
            >
                <div class="flex items-center gap-2 text-slate-400">
                    <span class="font-semibold tracking-[0.14em] text-slate-300"
                        >GAME TIME</span
                    >
                </div>
            ...
            <div class="flex-1 overflow-y-auto p-3">
                ...
                >
                    ...
                    <div class="flex flex-1 gap-2">
                        <div class="relative flex-1">
                            ...
                            <div
                                v-if="unlockReady"
                                class="pointer-events-none absolute right-3 top-3 text-amber-400"
                            >
                ...
                >
                    ...
                    <div
                        v-if="paletteMode === 'view'"
                        class="flex-1 space-y-1 overflow-y-auto pr-1"
                    >
                        <button
                        ...
                        >
                            ...
                            <Send
                                class="h-3 w-3 text-emerald-400 opacity-0 transition group-hover:opacity-100"
                            />
                        </button>
                        <p
                            v-if="paletteLines.length === 0"
                ...
                <div
                    v-show="activeTab === 'kp'"
                    class="h-full space-y-4 text-sm text-slate-100"
                >
                    <div class="grid h-full grid-cols-1 gap-4 md:grid-cols-2">
                        <div
                            class="flex flex-col gap-3 rounded-lg border border-slate-700 bg-slate-900/60 p-4"
                        >
                            <p
                            ...
                            >
                                "System"
                                はゲーム内時間の秒をシードにした決定論的な乱数を使用します。Quantum
                                モードを選ぶと外部量子乱数を利用します。
                            </p>
                        </div>
                        ...
                        >
                            ...
                            <div
                                class="rounded-lg border border-slate-700 bg-slate-900/70 px-3 py-2 text-xs text-slate-300"
                            >
                                <div class="flex items-center justify-between">
                                    <span class="text-slate-400"
                                        >Shared Stopwatch</span
                                    >
                                    <span
                                        class="font-mono text-sm"
                                        :class="
                                ...
                                <p class="mt-1 text-[11px] text-slate-500">
                                    全プレイヤー共通のゲーム時間です。必要に応じて一時停止や任意時刻の設定ができます。
                                </p>
                            </div>
                            <div class="space-y-1">
                                <label
                                    class="text-[11px] font-semibold uppercase tracking-[0.12em] text-slate-400"
                                    >Set Manual Time</label
                                >
                                <input
            ...
            <p v-if="chatError" class="px-4 pb-2 text-xs text-rose-400">
                {{ chatError }}
            </p>
        </footer>
    </div>
...
watch(displayName, (val) => {
    if (sessionId.value)
        localStorage.setItem(`kp-name-${sessionId.value}`, val);
});
...
type MessageWithResult = ApiMessage & {
...
async function loadSessionInfo() {
    ...
    try {
        const res = await axios.get(
            `${apiBase}/api/sessions/${sessionId.value}/info`
        );
...
function speakerClass(msg: MessageWithResult) {
    ...
    if (name.includes("kp") || name.includes("gm"))
        return "text-amber-300 font-semibold";
    if (name === effectiveName.value.toLowerCase())
...
function diceBadge(msg: MessageWithResult) {
    ...
    const base =
        target !== undefined
            ? `CC<=${target}`
            : msg.raw_text.split(" ")[0] || "ROLL";
    const status = res.result_level
        ? resultLabel(res.result_level)
        : res.success === true
        ? "成功"
        : res.success === false
        ? "失敗"
...
function badgeTone(msg: MessageWithResult) {
    ...
    if (level === "critical" || level === "extreme")
        return "border-emerald-400/60 text-emerald-200 bg-emerald-900/20";
    if (level === "fumble")
        return "border-rose-400/60 text-rose-200 bg-rose-900/20";
    if (res.success === true)
        return "border-emerald-400/60 text-emerald-200 bg-emerald-900/20";
